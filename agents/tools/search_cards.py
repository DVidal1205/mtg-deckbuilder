...
class CardSearchFilters:
    ...
    Fields:
      ...
      oracle_contains:
        Optional list of phrases to match in oracle_text (OR logic).
        Example: ["draw a card", "enters the battlefield"] matches cards with either phrase.
        Can also be a single string for backward compatibility.
    ...
    oracle_contains: Optional[Sequence[str]] = None
...
def search_cards(
    ...
    *,
    ...
    if f.oracle_contains:
        # Handle both single string (backward compatibility) and list of strings
        if isinstance(f.oracle_contains, str):
            oracle_phrases = [f.oracle_contains]
        else:
            oracle_phrases = list(f.oracle_contains)

        if oracle_phrases:
            clause, clause_params = _like_any("c.oracle_text", oracle_phrases)
            where.append(clause)
            params.extend(clause_params)
